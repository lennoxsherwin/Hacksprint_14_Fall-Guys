...
$stmt = $pdo->prepare(
  "SELECT song_id,song_name,song_path,artist.artist_name,artist.artist_id
   FROM songs
   INNER JOIN artist ON songs.artist_id = artist.artist_id
   WHERE songs.artist_id = :artist_id AND songs.effective_end_dt IS NULL
   ORDER BY song_name");
$stmt ->execute(array(
  ':artist_id' =>$q));
...
<tbody class="table_head" id="myTable">
