...
function showSongs(str) {
  ...
  document.getElementById('ajax_song_fields').style.display = "block";
   if (str == "") {
       ...
       xmlhttp.open("GET","get_songs_ajax.php?q="+str,true);
       xmlhttp.send();
   }
}
...
function pauseAudio() {
  ...
  function play_song(path,name,artist){
    event.preventDefault();
    document.getElementById('audio_player').setAttribute('src', path);
    document.getElementById("display_song_name").innerHTML = name;
    document.getElementById("display_artist_name").innerHTML = artist;
